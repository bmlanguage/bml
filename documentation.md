...
*  **Customizable String Interpolation:** Support for customizable string interpolation using the `{variable:formatSpecifier}` syntax for more control over output.
*   **Concise Bitfield Operations:** BML provides a concise syntax to access bitfields and bit ranges using `=`, `|=` and `&=`, and the `with` statement for combined bitfield updates, with support for direct bitfield setting, implicit bitfield access, and bit masks, which provides more flexibility and improves code readability. Bitfield operations inside a `with` statement benefit from specific optimizations that are performed by the compiler.
 ...
 *  **Implicit Type Conversions with `as`:** Allows type conversions between related data types (e.g., integers of the same sign or to floats) using the `as` keyword enhancing readability while still enforcing type safety. The compiler will not perform implicit conversion from signed to unsigned integers.
 *  **Inline Conditional Assignments:** Allows a compact syntax using `if(condition) value1 else value2` for conditional assignments.
    *  **Short Lambda functions**: The language supports short lambda functions using the syntax: `parameter => expression` to simplify function declarations.
    * **Implicit Register Declarations**: CAMs can now omit the register type when defining conceptual registers simplifying the code and making it easier to read.
    *  **Concise Region Definition**: Data regions with only one field can be defined by using the syntax `dataRegion MyData dataType myVariable;`.
 *  **Compile-Time String Processing**: The compiler supports intrinsic functions such as `stringLength()`, `stringReverse()`, `stringSub()`, `stringEquals()`, `stringConcat()` and `stringToInt()` that can be called during compile time and will perform operations on string literals.
 * **First Class Code Blocks:** Code blocks can be passed as parameters to functions to improve code composability.
 * **Implicit Return Type for Single Expression Functions:** Allows single-expression functions to omit the return type and infer it from the expression.
